...
//					canvas.drawText("第"+(pageNo-1)+"页", x+width-5-paint.measureText("第"+(pageNo-1)+"页"), y+height-5, paint);
	...
	private boolean isNearCorner(float hx,float hy){
		if(hx > width && hy > height/2){
			hx = 2 * width - 100;
			hy = height - 100;
		}
		else if(hx > width && hy < height/2){
			hx = 2 * width - 100;
			hy = 100;
		}
		else if(hx < width && hy < height/2){
	...
	private boolean isEnableChoose(float hx,float hy){
		...
		switch(chooseCorner){
		case LEFT_UP_CORNER:{
			if((hx-(width+x))*(hx-(width+x))+(hy-y)*(hy-y)<=width*width && (hx-(width+x))*(hx-(width+x))+(hy-(height+y))*(hy-(height+y))<=3*(width*width+height*height)){
				flag = true;
			}
			break;
		}
		case LEFT_BOTTOM_CORNER:{
			if((hx-(width+x))*(hx-(width+x))+(hy-(height+y))*(hy-(height+y))<=width*width && (hx-(width+x))*(hx-(width+x))+(hy-y)*(hy-y)<=3*(width*width+height*height)){
				flag = true;
			}
			break;
		}
...
//		Log.e("Page","isEnableChoose "+flag);
	...
	public void autoFlipPage(final float tx,final float ty,final boolean isSuccessFlip){
