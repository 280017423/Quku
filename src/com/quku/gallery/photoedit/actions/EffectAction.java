...
import com.quku.R;
import com.quku.gallery.photoedit.FilterStack;
import com.quku.gallery.photoedit.OnDoneCallback;
...
public abstract class EffectAction extends LinearLayout {
	...
	public void begin(FilterStack filterStack, EffectToolFactory factory) {
		// This view is already detached from UI view hierarchy by reaching
		// here; findViewById()
	...
	/**
	 * Ends the effect and then executes the runnable after the effect is
	 * finished.
	 */
	...
	protected void notifyFilterChanged(Filter filter, boolean output) {
		...
		if (pushedFilter && output) {
			// Notify the stack to execute the changed top filter and output the
			// results.
	...
	/**
	 * Subclasses should creates a specific filter and binds the filter to
	 * necessary UI controls here when the action is about to begin.
	 */
	protected abstract void doBegin();

	/**
	 * Subclasses could do specific ending operations here when the action is
	 * about to end.
	 */
