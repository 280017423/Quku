...
/**
 * Full-screen tool view that gets photo display bounds and maps positions on
 * photo display bounds back to exact coordinates on photo.
 */
abstract class FullscreenToolView extends View {
	...
	/**
	 * Photo bounds must be set before onSizeChanged() and all other instance
	 * methods are invoked.
	 */
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		// Assumes photo-view is also full-screen as this tool-view and
		// centers/scales photo to fit.
	...
	protected void mapPhotoRect(RectF src, RectF dst) {
		if (photoBounds.isEmpty()) {
			...
			dst.set(dst.left / photoBounds.width(), dst.top / photoBounds.height(), dst.right / photoBounds.width(),
