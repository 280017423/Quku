...
import com.quku.gallery.photoedit.filters.RotateFilter;
...
public class RotateAction extends EffectAction {
	...
	public void doBegin() {
		...
		rotateView.setOnRotateChangeListener(new RotateView.OnRotateChangeListener() {
			...
			// Directly transform photo-view because running the rotate filter
			// isn't fast enough.
			PhotoView photoView = (PhotoView) rotateView.getRootView().findViewById(R.id.photo_view);
	...
	public void doEnd() {
		...
		// Round the current rotation degrees in case rotation tracking has not
		// stopped yet.
		roundRotateDegrees();
