...
public class CropView extends FullscreenToolView {
	...
	private RectF getCropBoundsDisplayed() {
		...
		RectF cropped = new RectF(
				cropBounds.left * width, cropBounds.top * height, cropBounds.right * width, cropBounds.bottom * height);
	...
	private void moveEdges(float deltaX, float deltaY) {
		...
		if (movingEdges == MOVE_BLOCK) {
			...
			deltaX = (deltaX > 0) ? Math.min(displayBounds.right - cropped.right, deltaX) : Math.max(displayBounds.left
					- cropped.left, deltaX);
			deltaY = (deltaY > 0) ? Math.min(displayBounds.bottom - cropped.bottom, deltaY) : Math.max(
					displayBounds.top - cropped.top, deltaY);
	...
	protected void onDraw(Canvas canvas) {
		...
		// Draw shadow on non-cropped bounds and the border around cropped
		// bounds.
		RectF cropped = getCropBoundsDisplayed();
