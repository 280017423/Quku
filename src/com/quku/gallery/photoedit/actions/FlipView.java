...
public class FlipView extends FullscreenToolView {
	...
	public void setFlipSpan(float degrees) {
		// Flip-span limits allowed flipping degrees of every flip for usability
		// purpose; the max.
		// flipped angles could be accumulated and larger than allowed
		// flip-span.
		maxFlipSpan = degrees;
	}
		...
		if (isEnabled()) {
			switch (ev.getAction()) {
				...
				case MotionEvent.ACTION_MOVE:
					// Allow only one direction for flipping during movements,
					// and make the
					// direction fixed once it exceeds threshold.
					float x = ev.getX();
					float y = ev.getY();
					boolean flipHorizontal = fixedDirection ? fixedDirectionHorizontal : (Math.abs(x - touchStartX) >= Math
							.abs(y - touchStartY));
