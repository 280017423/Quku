...
public class DoodleView extends FullscreenToolView {
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if ((bitmap == null) && !r.isEmpty()) {
			...
			// Set up a matrix that maps back normalized paths to be drawn on
			// the bitmap or canvas.
			drawingMatrix.setRectToRect(new RectF(0, 0, 1, 1), r,
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (isEnabled()) {
			...
			case MotionEvent.ACTION_UP:
				// Line to last position with offset to draw at least dots for
				// single clicks.
