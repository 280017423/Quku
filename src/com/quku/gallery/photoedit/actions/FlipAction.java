...
import com.quku.R;
import com.quku.gallery.photoedit.PhotoView;
import com.quku.gallery.photoedit.filters.FlipFilter;
...
public class FlipAction extends EffectAction {
	...
	public void doBegin() {
		...
		flipView.setOnFlipChangeListener(new FlipView.OnFlipChangeListener() {
			...
			// Directly transform photo-view because running the flip filter
			// isn't fast enough.
			PhotoView photoView = (PhotoView) flipView.getRootView().findViewById(R.id.photo_view);
	...
	public void doEnd() {
		...
		// Round the current flip degrees in case flip tracking has not stopped
		// yet.
		roundFlipDegrees();
	...
	private void updateFlipFilter(boolean outputFilter) {
		// Flip the filter if the flipped degrees are at the opposite
		// directions.
