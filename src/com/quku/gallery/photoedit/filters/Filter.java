...
/**
 * Image filter for photo editing; most of its methods must be called from a
 * single GL thread except validate()/isValid() that are called from UI thread.
 */
public abstract class Filter implements Parcelable {
	...
	/**
	 * Some filters, e.g. lighting filters, are initially invalid until set up
	 * with parameters while others, e.g. Sepia or Posterize filters, are
	 * initially valid without parameters.
	 */
	...
	/**
	 * Processes the source bitmap and matrix and output the destination bitmap
	 * and matrix.
	 * 
	 * @param src
	 *            source photo as the input.
	 * @param dst
	 *            destination photo having the same dimension as source photo as
	 *            the output.
	 */
	...
	/**
	 * Saves states for restoring filter later; subclasses can override this to
	 * persist states.
	 */
	...
	/**
	 * Restores filter from the saved states; subclasses can override this to
	 * persist states.
	 */
