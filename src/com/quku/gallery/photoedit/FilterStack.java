...
import com.quku.gallery.photoedit.filters.Filter;
...
public class FilterStack {
	...
	// Use two photo buffers as in and out in turns to apply filters in the
	// stack.
	...
	private ArrayList<Filter> getFilters(Bundle savedState, String key) {
		// Infer Filter array-list from the Parcelable array-list by the
		// specified returned type.
	...
	private void invalidate() {
		...
		if (source != null) {
			...
			// Clear photo-view transformation when the top filter will be
			// outputted.
	...
	private int getOutBufferIndex(int filterIndex) {
		// buffers[0] and buffers[1] are swapped in turns as the in/out buffers
		// for
		// processing stacked filters. For example, the first filter reads
		// buffer[0] and
		// writes buffer[1]; the second filter then reads buffer[1] and writes
		// buffer[0].
		// The returned index should only be used when the applied filter stack
		// isn't empty.
	...
	public void onPause() {
		// Flush pending queued operations and release effect-context before GL
		// context is lost.
		// Use the flag to break from lengthy invalidate() in GL thread for not
		// blocking onPause().
		paused = true;
		photoView.flush();
		photoView.queueEvent(new Runnable() {
			...
			public void run() {
				...
				// Textures will be automatically deleted when GL context is
				// lost.
