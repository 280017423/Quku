...
public class PhotoView extends GLSurfaceView {
	...
	private class PhotoRenderer implements GLSurfaceView.Renderer {
		...
		void rotatePhoto(float degrees) {
			if (photo != null) {
				RendererUtils.setRenderToRotate(renderContext, photo.width(), photo.height(), viewWidth, viewHeight,
						degrees);
		...
		void flipPhoto(float horizontalDegrees, float verticalDegrees) {
			if (photo != null) {
				RendererUtils.setRenderToFlip(renderContext, photo.width(), photo.height(), viewWidth, viewHeight,
