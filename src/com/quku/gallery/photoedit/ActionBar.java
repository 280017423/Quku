...
public class ActionBar extends RestorableView {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		// Show the action-bar title only when there's still room for it;
		// otherwise, hide it.
		/*
		 * int width = 0; for (int i = 0; i < getChildCount(); i++) { width +=
		 * getChildAt(i).getWidth(); }
		 */
		// 屏蔽
		// findViewById(R.id.action_bar_title).setVisibility(((width > r - l)) ?
		// INVISIBLE: VISIBLE);
	}
	...
	/**
	 * Save/share button may need being switched when undo/save enabled status
	 * is changed/restored.
	 */
	private void showSaveOrShare() {
		// Show share-button only after photo is edited and saved; otherwise,
		// show save-button.
		boolean showShare = findViewById(R.id.undo_button).isEnabled() && !findViewById(R.id.save_button).isEnabled();
		ViewSwitcher switcher = (ViewSwitcher) findViewById(R.id.save_share_buttons);
		int next = switcher.getNextView().getId();
		if ((showShare && (next == R.id.share_button)) || (!showShare && (next == R.id.save_button))) {
			switcher.showNext();
		}
	}
