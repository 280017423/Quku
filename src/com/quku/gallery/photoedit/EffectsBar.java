...
import com.quku.R;
import com.quku.gallery.photoedit.actions.EffectAction;
import com.quku.gallery.photoedit.actions.EffectToolFactory;
...
public class EffectsBar extends LinearLayout {
	...
	public EffectsBar(Context context, AttributeSet attrs) {
		...
		// //SYSTEM_UI_FLAG_SHOW_FULLSCREEN = 0x00000004;//隐藏状态栏
	...
	public void initialize(FilterStack filterStack) {
		...
		effectsMenu.setOnToggleListener(new EffectsMenu.OnToggleListener() {
			...
			public boolean onToggle(boolean isSelected, final int effectsId) {
				// Create and show effects-gallery only if the clicked toggle
				// isn't selected or it's
				// selected but showing an active effect instead of
				// effects-gallery. Set the clicked
				// toggle selected only when its effects-gallery will be created
				// and shown.
	...
	private void createEffectsGallery(int effectsId) {
		// Inflate scrollable effects-gallery and desired effects into
		// effects-bar.
	...
	private void setupEffectListener(final EffectAction effect) {
		effect.setListener(new EffectAction.Listener() {
			...
			public void onClick() {
				if (isEnabled()) {
					// Set the clicked effect active before exiting
					// effects-gallery.
					activeEffect = effect;
					exitEffectsGallery();
					// Create effect tool panel first before the factory could
					// create tools within.
	...
	private boolean exitEffectsGallery() {
		if (effectsGallery != null) {
			if (activeEffect != null) {
				// Detach the active effect to prevent it stopping
				// effects-gallery from gc.
				ViewGroup scrollView = (ViewGroup) effectsGallery.findViewById(R.id.scroll_view);
	...
	/**
	 * Exits from effects gallery or the active effect; then executes the
	 * runnable if applicable.
	 * 
	 * @return true if exiting from effects gallery or the active effect;
	 *         otherwise, false.
	 */
