...
public class CameraActivity extends Activity implements SurfaceHolder.Callback {
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		if (null != mCam) {
			...
			mCam.startPreview();
			autoImage
					.setBackgroundResource(R.drawable.cam_focus_indicator_white_icn);
		}
	}
			...
			mCam.autoFocus(new AutoFocusCallback() {
				...
				public void onAutoFocus(boolean success, Camera arg1) {
					...
					if (success) {
						auto = true;
						autoImage
								.setBackgroundResource(R.drawable.cam_focus_indicator_green_icn);
					}
				}
			});
	...
	protected void onPause() {
		...
		try {
			...
			Log.d(SystemDef.Debug.TAG,
					TAG + " onPause exception = 	" + e.getMessage());
		}
		super.onPause();
	}
			...
			if (mCam != null) {
				...
				try {
					...
					Bitmap bitmap = Bitmap.createScaledBitmap(BitmapFactory
							.decodeByteArray(params[0], 0, params[0].length),
							SystemDef.System.IMG_PICTURE_HEIGHT,
							SystemDef.System.IMG_PICTURE_WIDTH, false);
					// 创建新bitmap
					Matrix m = new Matrix();
					...
					try {
						...
						/*
						 * if (hasStartPreview) { mCam.startPreview();
						 * autoImage.setBackgroundResource(R.drawable.
						 * cam_focus_indicator_white_icn); }
						 */
					} catch (Exception e) {
						e.printStackTrace();
					}
	...
	public void shootSound() {
		meng = (AudioManager) getBaseContext().getSystemService(
		...
		try {
			...
			Log.d(SystemDef.Debug.TAG,
					TAG + " shootSound exception = " + e.getMessage());
		}
	}
